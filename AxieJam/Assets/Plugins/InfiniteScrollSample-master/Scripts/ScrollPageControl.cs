...
public class ScrollPageControl : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public void Clear()
    {
        ...
        {
            //PoolController.Instance.DespawnObject(m_Toggles[i].transform);
            Destroy(m_Toggles[i].gameObject);
        }
        m_Toggles = new Toggle[0];
    }
    public void Setup()
    {
        if (m_IsSetupped)
        ...
        if (m_OriginalToggle != null)
        {
            ...
            {
                Toggle toggle = Instantiate<Toggle>(m_OriginalToggle, m_OriginalToggle.transform.parent);
                //var toggle = PoolController.Instance.SpawnObject(m_OriginalToggle.transform).GetComponent<SelectScreenToggle>();
                toggle.transform.SetParent(tfParent, false);
                //toggle.tmpPage.text = (i + 1).ToString();
                toggle.gameObject.SetActive(true);
                //toggle.toggle.interactable = false;// ぽっちは押せません
                toggle.interactable = false;
                m_Toggles[i] = toggle;// toggle.toggle;
            }
        }
    ...
    private IEnumerator ScrollInternal()
    {
        ...
        while (accum < duration)
        {
            ...
            var pos = new Vector2(
                Mathf.LerpUnclamped(from, to, curve.Evaluate(t)), m_Content.anchoredPosition.y);
        ...
        m_Content.anchoredPosition =
            new Vector2(to, m_Content.anchoredPosition.y);
    ...
    {
        if (m_PrevButton != null)
            m_PrevButton.interactable = (!limited || 0 < targetIndex);
        if (m_NextButton != null)
            m_NextButton.interactable = (!limited || targetIndex < length - 1);
